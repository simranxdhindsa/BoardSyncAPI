...
const AnalysisResults = ({ analysisData, selectedColumn, onBack, onSync, onCreateSingle, onCreateMissing }) => {
  ...
  // Handle individual ticket creation with selection-based approach
  const handleCreateTicket = async (task, index) => {
    const taskId = task.gid;
    setCreating(prev => ({ ...prev, [taskId]: true }));
    
    try {
      // Instead of calling createSingleTicket, we'll call a modified create endpoint
      // that accepts ticket selection indices or specific task data
      const response = await fetch(`${process.env.NODE_ENV === 'production' ? '' : 'http://localhost:8080'}/create-selected`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          selected_indices: [index], // Pass the index of this specific ticket
          // OR alternatively, pass the task data directly:
          selected_tasks: [task]
        })
      });
